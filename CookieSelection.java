...
public class CookieSelection {
    public static void main(String[] args) throws IOException {
        ...
        AVLTreeST<KeyValuePair, Boolean> avl = new AVLTreeST<>();
        String token = null;
        int count = 0;
        while ((token = br.readLine()) != null) {
            token = token.strip();
            if (token.equals("#")) {
                int median = avl.size() % 2 == 0 ? avl.size() / 2 + 1 : (avl.size() + 1) / 2;
                KeyValuePair node = avl.select(median - 1);
                int res = node.key;
                avl.delete(node);
                pw.println(res);
            } else {
                avl.put(new KeyValuePair(Integer.parseInt(token), count), true);
                count++;
            }
        }
